...
import prefect_streamline

ROOT_DIR = os.path.realpath(os.path.join(__file__, "..", ".."))
VERSION = importlib.metadata.version(prefect_streamline.__name__)
...
def publish():
    ...
    try:
        _, current_version, _ = alfred.run(git, ["describe", "--tags", "--abbrev=0"])
        ...
        if "fatal: No names found, cannot describe anything." in exception.stderr:
            ...
            raise
    _, git_status, _ = alfred.run(git, ["status"])
...
def publish__poetry():
    ...
    * POETRY_PYPI_TOKEN_PYPI
    ...
    """
    password = os.getenv('POETRY_PYPI_TOKEN_PYPI', None)

    if password is None:
        raise UsageError('POETRY_PYPI_TOKEN_PYPI must contain your pypi token : https://pypi.org/help/#apitoken')

    twine = alfred.sh("poetry")
    os.chdir(ROOT_DIR)
    alfred.run(twine, ['publish', '--build', '--non-interactive'])
